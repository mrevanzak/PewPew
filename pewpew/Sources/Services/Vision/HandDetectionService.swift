...
class HandDetectionService: ObservableObject {
  ...
  private func processHandDetectionResults() {
    ...
    results.forEach { observation in
      ...
      if observation.chirality == .left {
        ...
        let mcpKeys: [VNHumanHandPoseObservation.JointName] = [
  ...
  func getHandStates() -> [HandState] {
    ...
    {
      let isOpen = HandDetectionService.isHandOpen(
        fingerPoints: handDetectionData.leftFingerPoints, palm: leftPalm)
      states.append(isOpen ? .open : .closed)
    }
    if let rightPalm = handDetectionData.rightPalmPoint,
      handDetectionData.rightFingerPoints.count >= 5
    {
      let isOpen = HandDetectionService.isHandOpen(
        fingerPoints: handDetectionData.rightFingerPoints, palm: rightPalm)
      states.append(isOpen ? .open : .closed)
    }
    return states
  }
  ...
  func isHandOpen(hand: String) -> Bool {
    if hand == "left", let leftPalm = handDetectionData.leftPalmPoint,
      handDetectionData.leftFingerPoints.count >= 5
    {
      return HandDetectionService.isHandOpen(
        fingerPoints: handDetectionData.leftFingerPoints, palm: leftPalm)
    }
    if hand == "right", let rightPalm = handDetectionData.rightPalmPoint,
      handDetectionData.rightFingerPoints.count >= 5
    {
      return HandDetectionService.isHandOpen(
        fingerPoints: handDetectionData.rightFingerPoints, palm: rightPalm)
