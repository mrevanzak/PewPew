...
struct FingerPointsOverlay: View {
    ...
    private func drawHandSkeleton(context: GraphicsContext, hand: HandPoints, size: CGSize) {
        ...
        drawFingerChain(context: context, wrist: hand.wrist, fingerPoints: hand.thumb, size: size, color: .orange)
        
        // Draw index finger chain with connections
        drawFingerChain(context: context, wrist: hand.wrist, fingerPoints: hand.index, size: size, color: .yellow)
        
        // Draw middle finger chain with connections
        drawFingerChain(context: context, wrist: hand.wrist, fingerPoints: hand.middle, size: size, color: .green)
        
        // Draw ring finger chain with connections
        drawFingerChain(context: context, wrist: hand.wrist, fingerPoints: hand.ring, size: size, color: .blue)
        
        // Draw little finger chain with connections
        drawFingerChain(context: context, wrist: hand.wrist, fingerPoints: hand.little, size: size, color: .purple)
    }
    
    /// Draw finger chain with proper connections between joints
    private func drawFingerChain(context: GraphicsContext, wrist: CGPoint, fingerPoints: [CGPoint], size: CGSize, color: Color) {
    ...
    private func drawPoint(context: GraphicsContext, point: CGPoint, size: CGSize, color: Color, radius: CGFloat) {
        let circlePath = Path(
            ellipseIn: CGRect(
                x: point.x - radius,
                y: point.y - radius,
